...
<body>  
  ...
  <div class="ui container"> 
    ...
    <h3 class="ui center aligned dividing header">Registrar Servicio</h3>
    ...
    <form class="ui large form" method="POST" action="../../controlador/CServicio.php" id="formulario" name="formulario">
      ...
      <div class="two fields">
        ...
        <div class="required field">
          <label>Icono</label>
            <?php
              $tagcbotp=$combo->gen_combo_icon("SELECT * FROM cappiutep.t_icono WHERE estatus='1'", "id_icono","icono", isset($Serv['id_icono'])?$Serv['id_icono']:'','Icon',' class="ui search dropdown"');
      ...
      <div class="two fields">
        <div class="required field">
          ...
          <?php
             $tagcbotp=$combo->gen_combo("SELECT * FROM cappiutep.t_modulo WHERE estatus='1'", "id_modulo","descripcion", isset($Serv['id_modulo'])?$Serv['id_modulo']:'','Mod',' class="ui search dropdown"');
             foreach ($tagcbotp as $tagtp){echo $tagtp;}
          ?>
        </div>
      ...
      <div class="two fields">
        <div class="required field">
          ...
          <?php
             $tagcbotp=$combo->gen_combo("SELECT * FROM cappiutep.t_tipo_vista WHERE estatus='1'", "id_tipo_vista","descripcion", isset($Serv['id_tipo_vista'])?$Serv['id_tipo_vista']:'','TipVis',' class="ui search dropdown"');
             foreach ($tagcbotp as $tagtp){echo $tagtp;}
          ?>
        </div>
        ...
        <div class="field">
          ...
          <input type="text" id="URL" name="URL" maxlength="50" value="<?php if(isset($Serv['url'])) echo $Serv['posicion'];  ?>">
        </div>        
      </div>
