...
session_start();
  ...
  include('../../modelo/MCargosCajaAhorro.php');
  $OCargos = new Cargos;
  $OCargos -> setIdCargo($_GET['id']);
  $registro=$OCargos -> consultar();
  
  while ( $fila = $OCargos->setArreglo( $registro ))
      $datos[] = $fila;
    foreach ($datos as $Cargos) { $Cargos; }
  ...
  <div class="ui container"> 
    ...
    <form class="ui large form" method="POST" action="../../controlador/CCargosCajaAhorro.php" id="formulario" name="formulario">
      ...
      <div class="three fields">
        <div class="field">
          <label>Nombre</label>
          <input type="text" id="Nombre" name="Nombre" placeholder="Nombre" value="<?php echo $Cargos['nombre'] ?>" onkeypress="return soloLetras(event)">
        </div>
        <div class="field">
        </div>
        <div class="field">
          ...
          <div class="ui transparent input">
            <input type="text" id="FechaIni" name="FechaIni" value="<?php echo date('d/m/Y',strtotime( $Cargos['fecha_ini'] ))  ?>" readonly>
          </div>        
        </div>
      </div>
      ...
      <div class="field">
        ...
        <textarea name="Desc" id="Desc" rows="3" class="ui textarea"><?php echo $Cargos['descripcion'] ?></textarea>
      </div>
      ...
      <div class="three fields">
        ...
        <div class="field">
          ...
          <div class="field">
            <?php
            $tagcbotp=$combo->gen_combo("SELECT * FROM cappiutep.t_lista_valor WHERE estatus='1' AND id_lista=22", "id_lista_valor","nombre_largo", isset($Cargos['tipo_cargo'])?$Cargos['tipo_cargo']:'','TipoCargo',' class="ui dropdown"');
        ...
        <div class="field">
          ...
          <input type="number" id="MinPers" name="MinPers" onkeypress="return soloNumeros(event)" min="0" max="15" maxlength="2" value="<?php echo $Cargos['min_cant_personas'] ?>">
        </div>
        ...
        <div class="field">
          ...
          <input type="number" id="MaxPers" name="MaxPers" onkeypress="return soloNumeros(event)" min="0" max="9999" maxlength="4" value="<?php echo $Cargos['max_cant_personas'] ?>">
        </div>
 
      </div>      
      <div class="ui center aligned block inverted header">
        <input type="hidden" name="opera" id="opera">
        <input type="hidden" name="IdCargo" id="IdCargo" value="<?php echo $Cargos['id_cargo_caja'] ?>">
