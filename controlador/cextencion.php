<?php session_start();
	...
	$objBeneficio->desactivar_prestamo($_POST['soli']);
	
	if($objBeneficio->guardarExtencion($_POST['tipo'],$_POST['dni'])){//Si guarda correctamente mandamos el email	
	    ...
	    $Email  = $OSocio->getEmail($_SESSION["userActivo"]);
	  	...
	  	$mensaje = "
		        	Estimado Socio, le notificamos que su solicitud de Extencion de Préstamo Personal por el monto
			        ...
			        ";
        $mail->to($Email);
        $mail->tocc("monasterios.armando@gmail.com");
        $mail->toccname("Ing Monasterios");
