...
//--------------------------------------------------------------------
    ...
    {
        $sql="UPDATE cappiutep.t_municipio 
              	 SET estatus = '1'
...
//--------------------------------------------------------------------
    ...
    {
        $sql="SELECT * FROM cappiutep.t_municipio WHERE id_municipio= $this->IdMunicipio";
        return $this->consulta( $sql );
...
//--------------------------------------------------------------------
	...
	public function listar($IdPadre)	
	{
		...
		$sql="SELECT * FROM cappiutep.t_municipio WHERE id_estado=$IdPadre;";
		...
		while($row= $this->getArreglo($resulSet))		
		{
			...
			$MarcasSelect[$pos++]="<tr>
									 <td class='collapsing'>".$row['id_municipio']."</td>
									 <td>".$row['municipio']."</td>
									 <td class='collapsing'>".$status."</td>
									 <td class='collapsing'>
									    <a class='ui labeled icon button' data-content='Configurar' data-variation='basic' href='ConfigParroquia.php?id=".$row['id_municipio']."'>
											<i class='configure icon'></i>
											Configurar Parroquias
										</a>
									 </td>
									</tr>";
		}
